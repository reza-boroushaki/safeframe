...
const D = new U("MOBKOI"), P = D.enter("Celtra"), j = P.enter("VideoControllerOptions"), z = {
  video: "vidPlayer",
  btnSound: "btnSound",
...
class y {
  ...
  static expectInto(t, e = {}) {
    if (!t)
      return y.cancellableRejectedPromise(new Error("Cannot wait for video element without a container"));
    const n = y.findInto(t);
    if (n)
  ...
  static expectIntoWhenAppeared(t, e, n = {}) {
    ...
    const l = new Promise((c, u) => {
      a = () => {
        ...
        o && (r = y.expectInto(e, n), r.then(c).catch(u));
      });
    });
...
class R {
  ...
  setupViewportObserver() {
    ...
    if (!e)
      return;
    const n = e.getNode();
    y.expectIntoWhenAppeared(this.activationView, n, {
      onWaiting: () => {
        this.log.warn("Video element not found for viewport observer, waiting for it to be inserted");
      }
    }).then((i) => {
      console.log("delayedVideoElement*****", i), t === this.setupId && this.setupVisibilityObserver(i);
    }).catch((i) => {
      this.log.error(i);
    });
  }
...
class q {
  constructor(t) {
    this.log = t.enter("VideoSafeFrameHandler"), this.safeframeUtil = new W(this.log);
  }
  /**
...
class K {
  ...
  playIfAllowed() {
    if (this.setStatus("checking autoplay status"), V(
      ...
      const t = this.scope.find(this.options.video);
      t.autoplay && t.resetAction(this.actionCtx, {}, noop);
      const e = V(this.options, "showPlayBtnOnCtp");
  ...
  playCheck() {
    ...
    if (!n.hasAppearedAtLeastOnce && typeof n.on == "function") {
      n.on("appeared", () => {
        y.findInto(e) ? i() : this.waitForVideoElement(e, () => i());
      });
      return;
    }
  ...
  waitForVideoElement(t, e, n) {
    if (this.videoElementWait)
      return;
    const i = y.expectIntoWhenAppeared(n, t, {
      onWaiting: () => {
...
class Q {
  ...
  onPause(t) {
    t && (console.log("user paused*****************************", t), this.elementManager.showPlayButton()), this.setStatus("paused");
  }
  onMute() {
...
`.trim(), b = class b {
  ...
  onPlaying() {
    if (this.log.debug("onPlaying()"), console.log("onPlaying***************************"), this.eventHandlers.onPlaying(), this.celtraVideo.autoplay && this.options.scriptedPlay === !0 && !this.viewportObserver.inViewport && (this.options.scriptedPlay = "playing", this.scriptedPause()), this.videoCountdown) {
      const e = this.scope;
  ...
  scriptedPause() {
    console.log("scriptedPause()************"), this.log.debug("scriptedPause()"), this.suppressPlayButtonOnNextPause(), this.elementManager.hidePlayButton(), this.pause(() => {
      this.playWhenAppearing();
    });
  }
  onPause() {
    this.log.debug("onPause()"), console.log("onPause*****");
    const t = Date.now(), e = t - this.lastClickAt < b.USER_CLICK_WINDOW_MS && !this.suppressNextPausePlayButton;
    console.log("now - this.lastClickAt*****", t - this.lastClickAt), this.suppressNextPausePlayButton = !1, this.eventHandlers.onPause(e), this.videoCountdown && this.scope.pauseCountdown?.();
  }
  onTimeUpdate(t) {
  ...
  onEnded() {
    console.log("onEnded*****"), this.eventHandlers.onEnded(), this.videoCountdown && this.scope.resetCountdown?.();
  }
  onMute() {
  ...
  setupEventListeners() {
    ...
    e.on("playing", () => {
      ...
      if (console.log("celtra pause$$$$$$$"), this.hasVideoCompleted) {
        this.onEnded();
        return;
      }
      this.onPause();
    ...
    const n = e.getNode();
    y.expectIntoWhenAppeared(this.scope, n, {
      onWaiting: () => {
...
typeof window < "u" && (window.waitForCeltraGlobals = $, window.waitForCeltraGlobalsAnyWindow = st);
export {
  ...
  F as initializeOptions,
  dt as setup,
  $ as waitForCeltraGlobals,
